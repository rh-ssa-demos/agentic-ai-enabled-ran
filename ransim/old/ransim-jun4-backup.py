...
def inject_anomaly(kpi, usage, current_band_normal_throughput=None, current_band_normal_usage=None): # Add new params
    ...
    if anomaly_type == 'high_prb_utilization':
        ...
        # NEW LOGIC: Guarantee a drop compared to a 'normal' baseline
        # Use current_band_normal_throughput if available, otherwise use a safe average.
        ...
        kpi['Throughput (Mbps)'] = round(baseline_throughput * random.uniform(0.1, 0.4), 2) # Guarantee 60-90% drop
        if kpi['Throughput (Mbps)'] < 0.1: kpi['Throughput (Mbps)'] = 0.1 # Ensure not zero
    elif anomaly_type == 'ues_spike_drop':
        # NEW LOGIC: Guarantee >50% change
        ...
        if random.random() < 0.5: # 50% chance for drop
            usage = int(baseline_usage * random.uniform(0.1, 0.4)) # Drop to 10-40% of baseline
        else: # 50% chance for spike
            usage = int(kpi['Max Capacity'] * random.uniform(0.9, 1.0)) # Spike to 90-100% of max capacity
...
def simulate(cells, interval=60):
    ...
    num_anomaly_cells = int(num_total_cells * 0.30) # Calculate 30% of total cells
