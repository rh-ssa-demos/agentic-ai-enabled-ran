...
def genai_anomaly_detection(
    ...
    # insert event in th DB
    def insert_event_db_2(cell_id_ran, event_text, event_data, raw_csv_data):
        with engine.begin() as conn:
            conn.execute(events.insert().values(
                creation_date=datetime.utcnow(),
                cell_id=cell_id_ran,
    ...
    # Kafka producer for AAP integration
    KAFKA_TOPIC = "ai-ran-logs"
    try:
        producer = KafkaProducer(
            bootstrap_servers=['service1.pchzg.sandbox2169.opentlc.com:9092'],
            value_serializer=lambda v: json.dumps(v).encode('utf-8'),
            retries=5,
            linger_ms=10
        )
    except Exception as e:
        log.info("Failed to connect to Kafka for AAP integration, continue pipeline")
    ...
    **Important Rules:**
    ...
    api_fix_prompt = """
    You are an automation workflow assistant. Your task is to take the following anomaly information, interpret it, and generate **actionable steps** for the NOC engineer or automation system.
    ...
    Using the anomaly information above:

    1. **Extract key anomaly details**
       - Cell ID
       - The anomaly type (e.g., "Low RSRP")  
       - The measured value  
       - The expected threshold  
       - Any other relevant metric values in the message

    2. **Interpret the recommended fix**  
       - Determine WHICH parameters need to be modified (e.g., azimuth, downtilt, transmit power, etc.)  
       - If ambiguous, choose default engineering-safe values (e.g., azimuth +5°, downtilt +3°).
       - There can be only a maximum of 4 parameters in the JSON request, if you have to remove anomaly_type or anomaly_value you can do it.

    3. **Generate an actionable API call**  
       You must output:
       - A short, human-readable explanation of WHAT needs to be done
       - The JSON payload needed for the automation service, maximum of 4 parameters
       - The exact REST API request to execute, always use as endpoint https://ranchat-ai-cloud-ran-genai.apps.acmhub.dinesh154.dfw.ocp.run/api/ran
    ...
    ### ACTION REQUIRED  
    (Add some details about the anomaly)
    (One clear sentence summarizing what needs to happen.)
    ...
    **JSON Payload example:**  
    ```json
    {{
      "cell_id": "<always include the affected cell ID>"
      "anomaly_type": "<extracted anomaly type>",
      "anomaly_value": "<numeric value + units>",
      "threshold_value": "<extracted threshold>",
      "action": "<short description of the remediation>",
      "azimuth_change": "<if applicable, integer or null>",
      "downtilt_change": "<if applicable, integer or null>"
    }}
    ```
    """
    ...
    for record_dict in df_current_batch_for_records:
        ...
        if detected_anomalies_by_python: # If Python found anomalies, we expect LLM to format it.
            ...
            if match and "ANOMALY_DETECTED" in match.group(1):
                ...
                # Send anomaly to Kafka for AAP integration for automatic remediation
                # Get API parameters from LLM
                log.info("Extracting API parameters for remediation")
                aap_prompt = api_fix_prompt.format(anomaly_block=final_reason, llm_response=llm_raw_response)
                response = qa_chain.invoke(aap_prompt)
                result_raw = response["result"]

                try:
                    producer.send(KAFKA_TOPIC, result_raw)
                    producer.flush()
                    log.info(f"Published to Kafka topic '{KAFKA_TOPIC}': {result_raw}")
                except Exception as e:
                    log.info(f"Failed to publish kafka message: {e}", exc_info=True)
        ...
        log.info(f"============= Insert EVENT for {current_cell_id} in DB =================")
        insert_event_db_2(current_cell_id, "auto_detected_anomaly", llm_raw_response, df_current_batch.to_csv(index=False))
    ...
    try:
        ...
        log.info("Kafka not initialized can't close producer, continue")
...
if __name__ == "__main__":
    Compiler().compile(
        pipeline_func=ran_multi_prediction_pipeline_with_genai_aap_int,
        package_path="ran_multi_prediction_pipeline_with_genai_aap_int.yaml"
    )
    print("✅ Pipeline YAML 'ran_multi_prediction_pipeline_with_genai_aap_int.yaml' generated successfully.")
